...
class Curiosity(Exploration):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # change action_space from Tuple to MultiDiscret
        if not isinstance(action_space, (Discrete, MultiDiscrete)):
            action_space = MultiDiscrete([action_space[0].n, action_space[1].n])
    ...
    def _postprocess_torch(self, policy, sample_batch):
        ...
        dist_inputs = self.model._curiosity_inverse_fcnet(phi_cat_next_phi).to(self.device)
        action_dist = (
